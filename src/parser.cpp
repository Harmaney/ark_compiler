...
void init() {
  ...
  if (!input) {
    term_print.fatal() << "! " << strerror(errno) << std::endl;
    exit(0);
  }
  int mode = 0;
  std::string lhs;
...
void generate_table() {
  ...
  for (auto I : item_set) {
    for (auto it : I.first) {
      ...
      if (!next.empty()) {
        add_action(std::make_pair(SHIFT, item_set[go(I.first, *next.begin())]),
                   std::make_pair(I.second, *next.begin()));
      } else if (it.lhs != globalS) {
        add_action(std::make_pair(
  ...
  for (auto I : item_set) {
    for (auto a : terminal) {
      if (action_table.count(std::make_pair(I.second, a))) {
        of << I.second << " " << a << " "
           ...
           << action_table[std::make_pair(I.second, a)].second << std::endl;
      } else {
        if (parser_err_table.count(std::make_pair(I.second, a)))
          of << I.second << " " << a << " "
             << parser_err_table[make_pair(I.second, a)] << " -2" << std::endl;
      }
    }
    if (action_table.count(std::make_pair(I.second, "$"))) {
      of << I.second << " "
         ...
         << action_table[std::make_pair(I.second, "$")].second << std::endl;
    }
    for (auto A : non_terminal) {
      if (goto_table.count(std::make_pair(I.second, A))) {
        of << I.second << " " << A << " "
           << goto_table[std::make_pair(I.second, A)] << " -1" << std::endl;
      }
    }
  }
    ...
    if (_raw == "")
      *this = {"", "", "$", 0, 0};
      ...
      if (_type == "keyword" || _type == "punc") {
        ...
        else if (_raw == "<>" || _raw == ">" || _raw == "<" || _raw == ">=" ||
                 _raw == "<=")
          _parser_symbol = "relOP";
        else if (_raw == "*" || _raw == "/" || _raw == "div" || _raw == "mod" ||
                 _raw == "and")
        ...
        _parser_symbol = "ID";
      else if (_type == "intVal" || _type == "realVal" || _type == "stringVal")
...
AnalyseTreeNode* analyse(TokenQueue& tq) {
    ...
    auto update_properties = [&](AnalyseTreeNode* node) {
      if (node->type == "S") {  // S -> ProgramStruct
        ...
        node->prop = new std::remove_pointer<ProgramStruct>::type(
            vars, types, functions, cast<BlockAST*>((M["body"]).front()));
      } else if (node->type == "ProgramHead") {
        node->prop = nullptr;
      } else if (node->type == "ProgramBody") {  // ProgramBody -> Component
                                                 // CompoundStatement @
        ...
        } else if (node->son[0]->raw == "var") {  // Component -> var
                                                  // VarDeclaration ; Component
          prop = cast<Component>(node->son[3]->prop);
        ...
        if (node->son.size() == 3) {  // ConstDeclaration -> ID = ConstValue
          prop.push_back(new VariableDeclAST(
              new VariableExprAST(node->son[0]->raw), nullptr, false, true,
          ...
          prop.push_back(new VariableDeclAST(
              new VariableExprAST(node->son[2]->raw), nullptr, false, true,
        ...
        if (node->son.size() == 3) {  // TypeDeclaration -> ID = ActualType
          prop.push_back(new TypeDefAST(new BasicTypeAST(node->son[0]->raw),
                                        cast<ActualType>(node->son[2]->prop)));
        } else {  // TypeDeclaration -> TypeDeclaration ; ID =
                  // ActualType
          prop = cast<TypeDeclaration>(node->son[0]->prop);
          prop.push_back(new TypeDefAST(new BasicTypeAST(node->son[2]->raw),
        ...
        if (node->son.size() == 1) {  // ActualType -> Type
          ...
          prop = new StructDeclAST(rand_name(),
        ...
        if (node->son.size() == 3) {  // VarDeclaration -> IDList : Type
          for (std::string id : cast<IDList>(node->son[0]->prop)) {
            prop.push_back(new VariableDeclAST(new VariableExprAST(id),
                                               cast<Type>(node->son[2]->prop),
                                               false));
          }
          ...
          for (std::string id : cast<IDList>(node->son[2]->prop)) {
            prop.push_back(new VariableDeclAST(
                new VariableExprAST(id), cast<ActualType>(node->son[4]->prop),
                false));
          }
        }
        node->prop = prop;
        ...
        if (node->son.size() == 3) {  // Period -> Digits .. Digits
          prop = Period{std::make_pair(cast<Digits>(node->son[0]->prop),
        ...
        node->prop = prop;
      } else if (node->type == "Subprogram") {  // Subprogram -> SubprogramHead
                                                // ; SubprogramBody
        ...
        if (node->son.size() ==
          ...
          prop = new FunctionSignatureAST(
              node->son[1]->raw, cast<FormalParameter>(node->son[2]->prop),
        ...
        if (node->son.empty()) {
          ...
          prop = new BinaryExprAST(node->son[1]->raw,
          ...
          if (node->son[3]->type == "CompoundStatement") {
            ...
            // ElsePart
            prop =
                new IfStatementAST(cast<Expression>(node->son[1]->prop),
        ...
        if (node->son.size() > 0) {  // ElsePart -> else CompoundStatement
          if (node->son[1]->type == "CompoundStatement") {
            ...
            prop = new BlockAST({cast<Statement>(node->son[1]->prop)});
          }
          prop->set_row(node->son[0]->row);
        } else {  // ElsePart ->
          prop = new BlockAST({});
        ...
        */
        ...
        if (node->son.size() == 1)  // SimpleExpression -> Term
          ...
          prop = new BinaryExprAST(cast<std::string>(node->son[1]->prop),
                                   cast<ExprAST*>(node->son[0]->prop),
                                   cast<ExprAST*>(node->son[2]->prop));
          prop->set_row(node->son[1]->row);
        }
        ...
        if (node->son[0]->type == "Variable") {
          ...
          prop = new BinaryExprAST(".", cast<ExprAST*>(node->son[0]->prop),
  ...
  for (;;) {
    ...
    if (action_table.count(std::make_pair(cur_state, n.parser_symbol)) == 0) {
      ...
      if (parser_err_table.count(std::make_pair(cur_state, n.parser_symbol))) {
        ost << ": expecting " << rang::style::bold
            << parser_err_table[std::make_pair(cur_state, n.parser_symbol)]
            << rang::style::reset << " but " << rang::style::bold
            << n.parser_symbol << rang::style::reset << " found." << std::endl;
      } else {
        ost << "." << std::endl;
      }
      exit(0);
    ...
    if (act.first == SHIFT) {
      ...
      unlinked_nodes.push_back(
          new AnalyseTreeNode(n.raw, n.type, n.parser_symbol, n.row, n.column));
      (*unlinked_nodes.rbegin())->Report();
      assert(!tq.empty());
      n.load(tq.front());
      tq.pop();

      parser_action.push_back(
          {{"action", "shift"},
           {"tok", {n.raw, n.parser_symbol}},
           {"states", states},
           {"prefix", symbols},
           {"newnodes", {(uint64_t)(*unlinked_nodes.rbegin())}}});

    } else if (act.first == REDUCE) {
      Expr prod = production_content[act.second];
      do_reduce(prod);
      std::vector<uint64_t> newNodes;

      for (int i = last; i < parser_info.size(); ++i)
        newNodes.push_back(parser_info[i]["ID"]);
      parser_action.push_back({{"action", "reduce"},
                               {"reduce", act.second},
                               {"tok", {n.raw, n.parser_symbol}},
                               {"states", states},
                               {"prefix", symbols},
      ...
      parser_action.push_back({{"action", "acc"},
                               {"tok", {n.raw, n.parser_symbol}},
                               {"states", states},
                               {"prefix", symbols},
                               {"newnodes", {}}});
