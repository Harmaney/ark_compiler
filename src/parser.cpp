...
GrammarTreeNode* Analyse(string file_name) {
            ...
            if (node->type == "S") {  // S -> ProgramStruct
                ...
                if (node->son.size() == 3) { //ConstDeclaration -> ID = ConstValue
                    prop.push_back(new VariableDeclAST(
                        ...
                        false,
                        true,
                        evalConst(cast<ConstValue>(node->son[2]->prop))
                    ));
                }
                else { //ConstDeclaration -> ConstDeclaration ; ID = ConstValue
                    prop = cast<ConstDeclaration>(node->son[0]->prop);
                    prop.push_back(new VariableDeclAST(
                        new VariableExprAST(node->son[2]->raw),
                        nullptr,
                        false,
                        true,
                        evalConst(cast<ConstValue>(node->son[4]->prop))
                    ));
                }
                node->prop = prop;
            }
                ...
                if (node->son.size() == 3) { //TypeDeclaration -> ID = ActualType
                    prop.push_back(new TypeDefAST(
                        new BasicTypeAST(node->son[0]->raw),
                        cast<ActualType>(node->son[2]->prop)
                    ));
                }
                else { //TypeDeclaration -> TypeDeclaration ; ID = ActualType
                    prop = cast<TypeDeclaration>(node->son[0]->prop);
                    prop.push_back(new TypeDefAST(
                        new BasicTypeAST(node->son[2]->raw),
    ...
    std::ofstream parserOutFile("pof.json");
    parserOutFile << parserOutputer.dump();
