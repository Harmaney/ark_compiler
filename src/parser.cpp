    ...
    auto DoReduce = [&](Expr expr) {
        auto UpdateProperties = [&](GrammarTreeNode* node) {
            auto CreateBinaryAST = [&]() -> ExprAST* {
                return new BinaryExprAST(node->son[1]->raw,
            ...
            using Period =
                std::vector<std::pair<NumberExprAST*, NumberExprAST*>>;
            ...
            using SubprogramBody =
                std::pair<std::vector<VariableDeclAST*>, BlockAST*>;
            ...
            if (node->type == "S") {  // S -> ProgramStruct
                ...
                node->prop = new std::remove_pointer<ProgramStruct>::type(
                    vars, functions, cast<BlockAST*>((M["body"]).front()));
