...
void do_reduce(Production prod, vector<int> &State, vector<string> &Symbol,
               vector<AST *> &Ast_Node) {
    // cout<<"?????"<<endl;
    ...
    GrammarTreeNode new_Node =
        GrammarTreeNode(prod.first, prod.first, 0, 0, gid);
...
void analyse(string file_name) {
    ...
    do {
        now_state = *State.rbegin();
        //		cout<<"state:"<<now_state<<" str: "<<N.word<<" type:
        //"<<N.now_str<<endl;
        ...
        if (act.first == Shift) {
            ...
            // cout<<"shift"<<endl;
            GNode.push_back(
                GrammarTreeNode(N.now_str, N.type, N.row, N.column, ran()));
            N = get_newinput(inf);
