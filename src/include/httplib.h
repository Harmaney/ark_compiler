...
#ifdef _WIN32
        auto sock =
            WSASocketW(rp->ai_family, rp->ai_socktype, rp->ai_protocol, nullptr,
                       0, WSA_FLAG_NO_HANDLE_INHERIT | WSA_FLAG_OVERLAPPED);
        /**
         * Since the WSA_FLAG_NO_HANDLE_INHERIT is only supported on Windows 7
         * SP1 and above the socket creation fails on older Windows Systems.
         *
...
inline socket_t create_client_socket(
    ...
    auto sock = create_socket(
        ...
        [&](socket_t sock2, struct addrinfo &ai) -> bool {
            ...
            auto ret = ::connect(sock2, ai.ai_addr,
...

            {
                ...
                tv.tv_usec =
                    static_cast<decltype(tv.tv_usec)>(read_timeout_usec);
                setsockopt(sock2, SOL_SOCKET, SO_RCVTIMEO, (char *)&tv,
                           sizeof(tv));
            }
            {
                timeval tv;
                ...
                setsockopt(sock2, SOL_SOCKET, SO_SNDTIMEO, (char *)&tv,
                           sizeof(tv));
            }
...
class gzip_compressor : public compressor {
   public:
    gzip_compressor() {
        ...
        is_valid_ = deflateInit2(&strm_, Z_DEFAULT_COMPRESSION, Z_DEFLATED, 31,
...
class gzip_decompressor : public decompressor {
   public:
    gzip_decompressor() {
        ...
        // value to ensure that any gzip stream can be decoded. The offset of 32
        // specifies that the stream type should be automatically detected
   ...
   public:
    ...
    bool compress(const char *data, size_t data_length, bool last,
        ...
        for (;;) {
            ...
            if (output_bytes) {
                callback(reinterpret_cast<const char *>(buff.data()),
                         output_bytes);
            }
        }
    ...
    return prepare_content_receiver(
        ...
        [&](const ContentReceiverWithProgress &out) {
            ...
            if (is_chunked_transfer_encoding(x.headers)) {
                ...
                auto len =
                    get_header_value<uint64_t>(x.headers, "Content-Length");
                if (len > payload_max_length) {
                    exceed_payload_max_length = true;
                    skip_content_with_length(strm, len);
                    ret = false;
                } else if (len > 0) {
                    ret = read_content_with_length(strm, len,
                                                   std::move(progress), out);
                }
            }
...
inline bool write_content(Stream &strm, const ContentProvider &content_provider,
    ...
    return write_content(strm, content_provider, offset, length,
...
class MultipartFormDataParser {
   public:
    ...
    bool parse(const char *buf, size_t n,
...
bool process_multipart_ranges_data(const Request &req, Response &res,
    ...
    for (size_t i = 0; i < req.ranges.size(); i++) {
        ...
        stoken(
            make_content_range_header_field(offset, length, res.body.size()));
        ctoken("\r\n");
        ctoken("\r\n");
...
inline std::string SHA_256(const std::string &s) {
    return message_digest<SHA256_CTX>(s, SHA256_Init, SHA256_Update,
                                      SHA256_Final, SHA256_DIGEST_LENGTH);
}

inline std::string SHA_512(const std::string &s) {
    return message_digest<SHA512_CTX>(s, SHA512_Init, SHA512_Update,
    ...
    if (n >= buf.size() - 1) {
        ...
        while (n >= glowable_buf.size() - 1) {
            ...
            n = static_cast<size_t>(snprintf(
                &glowable_buf[0], glowable_buf.size() - 1, fmt, args...));
#endif
        }
    ...
    if (res.content_length_ > 0) {
        if (req.ranges.empty()) {
            ...
            auto offsets = detail::get_range_offset_and_length(
                req, res.content_length_, 0);
            auto offset = offsets.first;
            auto length = offsets.second;
            return detail::write_content(strm, res.content_provider_, offset,
        ...
        if (res.is_chunked_content_provider_) {
            ...
            return detail::write_content_without_length(
                strm, res.content_provider_, is_shutting_down);
        }
    }
}
...
inline bool Server::read_content_core(Stream &strm, Request &req, Response &res,
    ...
    if (req.is_multipart_form_data()) {
        ...
        out = [&](const char *buf, size_t n, uint64_t /*off*/,
                  uint64_t /*len*/) {
            /* For debug
            size_t pos = 0;
            while (pos < n) {
        ...
        while (svr_sock_ != INVALID_SOCKET) {
            ...
            if (sock == INVALID_SOCKET) {
                if (errno == EMFILE) {
                    ...
                    // reached. Try to accept new connections after a short
                    // sleep.
                    std::this_thread::sleep_for(std::chrono::milliseconds(1));
                    continue;
                }
...

            {
                ...
                tv.tv_usec =
                    static_cast<decltype(tv.tv_usec)>(read_timeout_usec_);
            ...
            }
            {
                ...
                tv.tv_usec =
                    static_cast<decltype(tv.tv_usec)>(write_timeout_usec_);
...
inline void Server::apply_ranges(const Request &req, Response &res,
    ...
    if (res.body.empty()) {
        ...
        if (req.ranges.empty()) {
            ...
            std::string data;
            if (detail::make_multipart_ranges_data(req, res, boundary,
                                                   content_type, data)) {
                res.body.swap(data);
            } else {
                res.body.clear();
        ...
        if (type != detail::EncodingType::None) {
            ...
            if (compressor) {
                ...
                if (compressor->compress(
                        res.body.data(), res.body.size(), true,
                        [&](const char *data, size_t data_len) {
...
inline bool ClientImpl::send(Request &req, Response &res, Error &error) {
    ...
    {
        std::lock_guard<std::mutex> guard(socket_mutex_);
        // Set this to false immediately - if it ever gets set to true by the
        // end of the request, we know another thread instructed us to close the
        // socket.
        ...
        if (socket_.is_open()) {
            ...
            if (!is_alive) {
                // Attempt to avoid sigpipe by shutting down nongracefully if it
                // seems like the other side has already closed the connection
                // Also, there cannot be any requests in flight from other
                // threads since we locked request_mutex_, so safe to close
        ...
        // Mark the current socket as being in use so that it cannot be closed
        // by anyone else while this request is ongoing, even though we will be
        // releasing the mutex.
        if (socket_requests_in_flight_ > 1) {
            assert(socket_requests_are_from_thread_ ==
            ...
            if (detail::parse_www_authenticate(res, auth, is_proxy)) {
                ...
                new_req.headers.insert(
                    detail::make_digest_authentication_header(
                        req, auth, new_req.authorization_count_,
                        detail::random_string(10), username, password,
                        is_proxy));
...
inline bool ClientImpl::write_content_with_provider(Stream &strm,
    ...
    if (req.is_chunked_content_provider_) {
        ...
        return detail::write_content_chunked(
            strm, req.content_provider_, is_shutting_down, *compressor, error);
    } else {
...
inline bool ClientImpl::write_request(Stream &strm, Request &req,
    ...
    // Request line and headers
    {
        ...
        const auto &path =
            url_encode_ ? detail::encode_url(req.path) : req.path;
...
inline bool ClientImpl::process_request(Stream &strm, Request &req,
    ...
    if ((res.status != 204) && req.method != "HEAD" &&
        ...
        if (!detail::read_content(
                strm, res, (std::numeric_limits<size_t>::max)(), dummy_status,
    ...
    if (res.get_header_value("Connection") == "close" ||
        ...
        // TODO this requires a not-entirely-obvious chain of calls to be
        // correct for this to be safe. Maybe a code refactor (such as moving
        // this out to the send function and getting rid of the recursiveness of
        // the mutex) could make this more obvious.
        ...
        // mutex during the process. It would be a bug to call it from a
        // different thread since it's a thread-safety issue to do these things
        // to the socket if another thread is using the socket.
...
inline Result ClientImpl::Post(const char *path, const Headers &headers,
                               ContentProviderWithoutLength content_provider,
                               const char *content_type) {
    return send_with_content_provider("POST", path, headers, nullptr, 0,
...
inline Result ClientImpl::Put(const char *path, const Headers &headers,
                              const char *body, size_t content_length,
                              const char *content_type) {
    return send_with_content_provider("PUT", path, headers, body,
                                      content_length, nullptr, nullptr,
                                      content_type);
}
...
inline Result ClientImpl::Patch(const char *path, const Headers &headers,
                                ContentProviderWithoutLength content_provider,
                                const char *content_type) {
    return send_with_content_provider("PATCH", path, headers, nullptr, 0,
                                      nullptr, std::move(content_provider),
                                      content_type);
}
...
template <typename T>
inline bool process_server_socket_ssl(SSL *ssl, socket_t sock,
                                      size_t keep_alive_max_count,
                                      time_t keep_alive_timeout_sec,
                                      time_t read_timeout_sec,
                                      time_t read_timeout_usec,
...
inline bool SSLSocketStream::is_readable() const {
    return detail::select_read(sock_, read_timeout_sec_, read_timeout_usec_) >
           0;
}

inline bool SSLSocketStream::is_writable() const {
    return detail::select_write(sock_, write_timeout_sec_,
                                write_timeout_usec_) > 0;
}
...
inline SSLServer::SSLServer(const char *cert_path, const char *private_key_path,
    ...
    if (ctx_) {
        ...
        if (SSL_CTX_use_certificate_chain_file(ctx_, cert_path) != 1 ||
            SSL_CTX_use_PrivateKey_file(ctx_, private_key_path,
                                        SSL_FILETYPE_PEM) != 1) {
            SSL_CTX_free(ctx_);
            ctx_ = nullptr;
...
inline bool SSLServer::process_and_close_socket(socket_t sock) {
    ...
    if (ssl) {
        ...
        // Shutdown gracefully if the result seemed successful, non-gracefully
        // if the connection appeared to be closed.
        ...
        if (!proxy_digest_auth_username_.empty() &&
            ...
            if (detail::parse_www_authenticate(res2, auth, true)) {
                ...
                if (!detail::process_client_socket(
                    ...
                    // Thread-safe to close everything because we are assuming
...
inline Client::Client(const char *scheme_host_port,
    ...
    if (std::regex_match(scheme_host_port, m, re)) {
        ...
        auto port_str = m[3].str();
        auto port =
            !port_str.empty() ? std::stoi(port_str) : (is_ssl ? 443 : 80);

        if (is_ssl) {
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
            cli_ = detail::make_unique<SSLClient>(
                host.c_str(), port, client_cert_path, client_key_path);
            is_ssl_ = is_ssl;
#endif
        } else {
        ...
        cli_ = detail::make_unique<ClientImpl>(
            scheme_host_port, 80, client_cert_path, client_key_path);
    }
}
...
inline Result Client::Post(const char *path, const Headers &headers,
                           ...
                           ContentProvider content_provider,
                           const char *content_type) {
    return cli_->Post(path, headers, content_length,
...
inline Result Client::Patch(const char *path, const Headers &headers,
                            ...
                            ContentProvider content_provider,
                            const char *content_type) {
    return cli_->Patch(path, headers, content_length,
                       std::move(content_provider), content_type);
}
inline Result Client::Patch(const char *path, const Headers &headers,
                            ContentProviderWithoutLength content_provider,
                            const char *content_type) {
    return cli_->Patch(path, headers, std::move(content_provider),
                       content_type);
}
