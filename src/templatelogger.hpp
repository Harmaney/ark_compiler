...
template <typename List>
void log(std::ostream &os, const char *file, int line,
         const LogData<List> &data) {
    os << file << " (" << line << "): ";
    printList(os, data.list);
    os << "\n";
}

template <typename List>
void log(const char *file, int line, const LogData<List> &data) {
    log(std::cout, file, line, data);
}

#define LOG(x) (log(__FILE__, __LINE__, LogData<None>() << x))

#define OLOG(os, x) (log((os), __FILE__, __LINE__, LogData<None>() << x))
